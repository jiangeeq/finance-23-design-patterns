 ...
 *  --在代理工厂模式中，我们使用一个工厂类来创建代理实例。工厂类负责实例化实际对象和代理对象，并根据需要设置代理的行为。
 *  --代理工厂模式使得代理的创建更加灵活和可配置
 *
 * 关键点：
...
public class Main {
    public static void main(String[] args) {
        // 使用代理工厂创建股票价格服务的代理实例
        StockPriceService cacheProxy = StockPriceServiceProxyFactory.createCacheProxy();
        StockPriceServiceDBProxy dbProxy = StockPriceServiceProxyFactory.createDBProxy();

        // 第一次访问，缓存未命中
        queryStockPrice("alibaba", cacheProxy);
        // 第二次访问，缓存命中
        queryStockPrice("alibaba", cacheProxy);
        // 第一次访问，数据库未命中
        queryStockPrice("baidu", dbProxy);
        // 第二次访问，数据库命中
        queryStockPrice("baidu", dbProxy);
    }
